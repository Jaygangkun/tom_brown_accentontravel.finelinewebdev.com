...
class Qlue404ModelQlue404 extends JModelItem {
	...
	public function populateState() {
		...
		$this->setState('qlue404.id', (int)$id);
	}
	...
	public function getItem($id = null) {
		...
		if ($this->_item === null) {
			...
			JTable::addIncludePath(JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_qlue404'.DIRECTORY_SEPARATOR.'tables');
			...
			if($id && (JRequest::getCmd('option') == 'com_qlue404')) {
				...
				$item = array('error' => '%'. $this->getState('qlue404.error.code') .'%');
				$operator = 'LIKE';
			}
			
			// Attempt to load the row.
			if ($table->load($item, $operator)) {
			...
			} else if(JFile::exists(JPATH_ROOT.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_qlue404'.DIRECTORY_SEPARATOR.'error'.DIRECTORY_SEPARATOR.'error.php')) {
	...
	protected function _createError() {
		...
		require_once JPATH_ROOT.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_qlue404'.DIRECTORY_SEPARATOR.'error'.DIRECTORY_SEPARATOR.'error.php';
