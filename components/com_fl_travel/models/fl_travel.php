...
class FLTravelModelfl_travel extends JModelLegacy
{
	function getAll($startAt = 1, $maxReturn = 24, $filter = "", $include = "", $sort = "", $sortOrder = "") {
		$sigConnect = new sigConnect();
		
		return $sigConnect->getCruiseOfferPage($startAt, $maxReturn, $filter, $include, $sort, $sortOrder);
	}
	...
	function getOne($offerId) {
		...
		$offer = $sigConnect->getCruiseOffer($offerId);
		
		$query = "SELECT * FROM #__fl_travel_ships WHERE ship_id = $offer->ship_id";
		$db->setQuery($query);
		$ship = $db->loadObject();
		
		$query = "SELECT * FROM #__fl_travel_suppliers WHERE supplier_id = $offer->supplier_id";
		$db->setQuery($query);
	...
	function getAllShips() {
		...
		$query = "SELECT ship_id, ship_name, ship_photo, ship_description FROM #__fl_travel_ships";
		$db->setQuery($query);
		$getAll = $db->loadObjectList();
		
		$allShips = array();
		foreach($getAll as $ship) {
			$allShips[$ship->ship_id] = $ship;
		}
	...
	function getAllDestinations() {
		...
		$query = "SELECT destination_id, destination_name, parent_id FROM #__fl_travel_destinations";
		$db->setQuery($query);
		$getAll = $db->loadObjectList();
		
		$allDestinations = array();
		foreach($getAll as $dest) {
			$allDestinations[$dest->destination_id] = $dest;
		}
				...
				// } else {
					...
					// if($searchType == "select") {
						// $searchWheres[] = "1 = (
							...
							// INNER JOIN #__fl_items_option io ON io.item_property_id = ip.item_property_id AND io.option = '" . $propertyValue . "'
...
// 		
		// if($sortBy) {
			...
			// }
			// if($split[0] == "name") {
				// $query .= "ORDER BY i.isFeatured DESC, i.name $split[1], i.ordering, i.item_id";
			...
			// } else if($split[0]) {
				...
				// if($type == "number" || $type == "price") {
					// $query .= "ORDER BY i.isFeatured DESC, CAST((SELECT iip.value 
						// FROM #__fl_items_item_property iip
						// INNER JOIN #__fl_items_properties ip ON ip.name = '" . $split[0] . "'
						// WHERE iip.item_id = i.item_id AND iip.item_property_id = ip.item_property_id) AS UNSIGNED) ".$split[1].", i.ordering, i.item_id";
				// } else {
					// $query .= "ORDER BY i.isFeatured DESC, (SELECT iip.value 
						// FROM #__fl_items_item_property iip
						// INNER JOIN #__fl_items_properties ip ON ip.name = '" . $split[0] . "'
						// WHERE iip.item_id = i.item_id AND iip.item_property_id = ip.item_property_id) ".$split[1].", i.ordering, i.item_id";
				// }
			// } else {
