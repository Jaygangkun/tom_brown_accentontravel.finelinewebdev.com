...
class JMapSeostatsServicesGoogleSearch extends JMapSeostats {
	...
	protected static function makeRequest($pageNumber, $query, $result, $customHeaders, $onlyIndexedCount = false) {
		...
		$linkIndex = 0;
		foreach ( $simpleHtmlDomInstance->find( 'div.g > div > div > a, div.g div.rc > div > a' ) as $element ) {
		...
		foreach ( $simpleHtmlDomInstance->find( 'div.g > div > div > a > h3, div.g div.rc > div > a > h3' ) as $index=>$element ) {
			$titles[$index] = html_entity_decode($element->innertext, ENT_QUOTES, 'UTF-8');
		}
		foreach ( $simpleHtmlDomInstance->find( 'div.g > div > div > div > span, div.g div.rc > div > div > span' ) as $index=>$element ) {
	...
	public static function getRankedPageKeyword($query, $domain, $pageNumber = 0, $numResults = 100, $customHeaders = array()) {
		...
		foreach ( $simpleHtmlDomInstance->find( 'div.g > div > div > a, div.g div.rc > div > a' ) as $index=>$element ) {
