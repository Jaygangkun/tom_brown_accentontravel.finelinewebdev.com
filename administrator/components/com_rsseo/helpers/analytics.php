...
class rsseoGoogleAnalytics {
	...
	public function __construct() {
		...
		$client->setClientId(trim($config->analytics_client_id));
		$client->setClientSecret(trim($config->analytics_secret));
		$client->setRedirectUri(JURI::root().'administrator/index.php?option=com_rsseo&task=analytics.connect');
	...
	public static function getInstance() {
		...
		if (!$instance) {
			$instance = new rsseoGoogleAnalytics;
		}
	...
	public static function getToken() {
		...
		return $session->get('rsseo.access_token','');
	}
	...
	public static function authenticate() {
		...
		if ($code) {
			if (self::$client->authenticate($code)) {
				$session = JFactory::getSession();
				$session->set('rsseo.access_token', self::$client->getAccessToken());
			}
		}
	}
