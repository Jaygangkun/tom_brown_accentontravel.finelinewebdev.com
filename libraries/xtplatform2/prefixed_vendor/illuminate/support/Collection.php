<?php
/* This file has been prefixed by <PHP-Prefixer> for "XT Tailwind CSS" on 2019-08-30 11:38:40 */
...
use XTP_BUILD\Illuminate\Contracts\Support\Jsonable;
use XTP_BUILD\Illuminate\Contracts\Support\Arrayable;
...
class Collection implements ArrayAccess, Arrayable, Countable, IteratorAggregate, Jsonable, JsonSerializable
{
    ...
    public function median($key = null)
    {
        ...
        $values = XTP_with(isset($key) ? $this->pluck($key) : $this)
                    ->sort()->values();
    ...
    public function containsStrict($key, $value = null)
    {
        if (func_num_args() == 2) {
            return $this->contains(function ($item) use ($key, $value) {
                return XTP_data_get($item, $key) === $value;
            });
        }
    ...
    protected function operatorForWhere($key, $operator, $value)
    {
        return function ($item) use ($key, $operator, $value) {
            $retrieved = XTP_data_get($item, $key);
    ...
    public function whereIn($key, $values, $strict = false)
    {
        ...
        return $this->filter(function ($item) use ($key, $values, $strict) {
            return in_array(XTP_data_get($item, $key), $values, $strict);
        });
    }
    ...
    protected function valueRetriever($value)
    {
        ...
        return function ($item) use ($value) {
            return XTP_data_get($item, $value);
        };
    }
