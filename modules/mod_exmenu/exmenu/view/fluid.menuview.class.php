...
class FluidExtendedMenuView extends AbstractExtendedMenuView {
	/* menu link classes*/
	function getMenuClassName($menuNode, $level = 0, $activeMenuClass = TRUE) {
		if ($level > 0) {
			$menuClass = 'nav-link sublevel';
		} else {
			$menuClass = 'nav-link mainlevel';
		}
...
//					$txt = '<a href="'. $menuNode->link .'" class="'. $menuclass .'"'. $elementParameters .'>'. $menuNode->name .'</a>';
	...
	function _renderMenuNodeList($menuNodeList, $level = 0, $hierarchy = array(), $noLineMap = NULL, $parentItemNumber = 0) {
		...
		$iItem = 0;
		if ($level == 0) {
			$result	.= '<div class="collapse navbar-collapse '.$this->classSuffix.'" id="navbarContent">';
		}
		$result	.= '<ul';
		if ($level == 0) {
			$result	.= ' id="'.$this->idSuffix.'" class="navbar-nav" role="menu" itemscope itemtype="http://www.schema.org/SiteNavigationElement"'; 
		}
		else
		{
		...
		foreach($keys as $id) {
			...
			$result	.= '<li id="'.$thisLIID.'" itemprop="name" class="nav-item';
			if ($menuNode->isCurrent()) {
				$result	.= ' active';
			}
