	...
	jQuery.ajax({
		...
		if (typeof RSSeo.isRefresh != 'undefined') {
			...
			if (response.status == 200) {
				jQuery('#status'+response.id).removeClass('badge-important').addClass('badge-success');
			} else {
				jQuery('#status'+response.id).removeClass('badge-success').addClass('badge-important');
			}
...
RSSeo.checkPage = function(id, original) {
	...
	jQuery.ajax({
		...
		if (response.status == 200) {
			jQuery('#status'+id).removeClass('badge-important').addClass('badge-success');
		} else {
			jQuery('#status'+id).removeClass('badge-success').addClass('badge-important');
		}
	});
}
...
RSSeo.createSitemap = function(isnew) {
	...
	jQuery.ajax({
		...
		data: 'task=sitemap.generate&new='+isnew+'&protocol='+protocol+'&modified='+modified+'&auto='+auto+'&port='+port
	}).done(function( response ) {
		if (response.status == 1) {
...
RSSeo.showPages = function(id, date) {
	...
	jQuery('#rsseo-pages').find('.modal-content,.modal-body').html('');
	jQuery('#rsseo-pages').on('show.bs.modal', function (e) {
		jQuery(this).find('.modal-content,.modal-body').load('index.php?option=com_rsseo&view=gkeyword&layout=edit&tpl=page&tmpl=component&id=' + id + '&date=' + date);
	});
